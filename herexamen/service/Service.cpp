...
string Service::processMessages(string &recvMsg) {
        ...
        switch (getCommandType(berichtInList[1])) {
            ...
            case CommandType::PostPone:
                    ...
                    return PostPone(berichtInList[2].toStdString(),berichtInList[3].toStdString());
...
string Service::PostPone(const string &Taskname, const string& NewDate){

    for (int i = 0; i < Tasks[ClientID].size(); ++i) {

        if (Tasks[ClientID][i].getName() == Taskname) {

            string temp = Tasks[ClientID][i].getDate();
            Tasks[ClientID][i].setDate(NewDate);

           return " TaskData veranderd " + temp + " to " + NewDate;;
        }
    }return " error ";
}
